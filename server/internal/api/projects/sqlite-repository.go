...
func (r *ProjectRepo) GetByID(ctx context.Context, id string, userID string) (*Project, error) {
	...
	err := r.db.QueryRowContext(ctx, query, id, userID).Scan(&project.ID, &project.Name, &project.CreatedBy, &project.CreatedAt, &project.UpdatedAt)
...
func (r *ProjectRepo) ListByUser(ctx context.Context, userID string) ([]Project, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&project.ID, &project.Name, &project.CreatedBy, &project.CreatedAt, &project.UpdatedAt)
