...
// TODO: Finish decoupling validators and encryption from the use cases. It's ok for MVP, but they need to be injected as dependencies for ease of testing.

// @UseCase Creates a new environment for a project.
func CreateProjectEnvironment(ctx context.Context, input CreateEnvironmentRequest, masterKey []byte, repo Repository, validator ConnectionValidator) error {

	// Always use dependency injection to deal with the connections. The handlers can use Postgres, however, the mocks can use a different strategy.
	dbVersion, err := validator.ValidateConnection(ctx, input.ConnectionUrl)
